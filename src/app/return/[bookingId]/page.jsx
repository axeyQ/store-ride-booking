...
{/* Updated Return Summary Header - Handles both pricing types */}
  ...
  <div className="grid grid-cols-1 md:grid-cols-4 gap-6 text-center">
    ...
    <div>
      ...
      <div className="text-green-100">
        {booking.isCustomBooking ? 'Package Price' : 'Advanced Pricing'}
      </div>
      <div className="text-xs text-green-200 mt-1">
        {booking.isCustomBooking ? 'Fixed rate package' : 'System calculated'}
      </div>
    </div>
    <div>
...
{/* Updated UPI QR Code Section - Shows correct amount for custom bookings */}
{returnData.paymentMethod === 'upi' && (
  <div className="mt-8 p-6 bg-gradient-to-r from-blue-900/50 to-blue-800/50 border border-blue-500/50 rounded-xl">
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="space-y-2 text-sm">
            ...
            {booking.isCustomBooking && (
              <div className="flex justify-between">
                ...
                <span className="text-purple-300 text-xs">{booking.customBookingType}</span>
              </div>
            )}
          </div>
        </div>
              ...
              <span className="text-white font-medium">
                ...
                {booking.isCustomBooking && (
                  <span className="block text-sm text-purple-300">
                    📦 {booking.customBookingType} package payment
                  </span>
                )}
              </span>
            </label>
      ...
      {/* Updated Payment Breakdown - Handles both pricing types */}
<div className={cn(
  "border border-opacity-50 rounded-xl p-8 mt-8",
  booking.isCustomBooking 
  ...
  <h3 className={cn(
    ...
    {booking.isCustomBooking ? "📦 Custom Package Breakdown" : "📊 Advanced Payment Breakdown"}
  </h3>
  ...
  <div className="space-y-4">
    {booking.isCustomBooking ? (
      ...
      <>
        <div className="flex justify-between text-lg">
          <span className="text-purple-200">Package Type:</span>
          <span className="font-semibold text-white capitalize">
            {booking.customBookingLabel || `${booking.customBookingType} Package`}
          </span>
        </div>
        <div className="flex justify-between text-lg">
          <span className="text-purple-200">Fixed Package Price:</span>
          <span className="font-semibold text-white">₹{booking.finalAmount?.toLocaleString('en-IN')}</span>
        </div>
        <div className="flex justify-between text-lg">
          <span className="text-purple-200">Duration Used:</span>
          <span className="font-semibold text-white">{duration.hours}h {duration.minutes}m</span>
        </div>
        
        {/* Package Details */}
        <div className="bg-purple-800/30 rounded-lg p-4">
          <h5 className="font-semibold text-purple-200 mb-3">Package Details:</h5>
          <div className="text-sm text-purple-100">
            <div className="mb-2">
              <span className="text-purple-300">Package:</span> {booking.customBookingType}
            </div>
            <div className="mb-2">
              <span className="text-purple-300">Duration:</span> {duration.totalMinutes} minutes used
            </div>
            <div className="text-xs text-purple-300 mt-3">
              📦 Fixed rate package - no additional charges based on time
            </div>
          </div>
        </div>
      </>
    ...
    <div className={cn(
      "border-t pt-4 flex justify-between text-2xl font-bold",
      booking.isCustomBooking ? "border-purple-600" : "border-blue-600"
    )}>
      <span className={booking.isCustomBooking ? "text-purple-200" : "text-blue-200"}>
        Final Amount:
      </span>
